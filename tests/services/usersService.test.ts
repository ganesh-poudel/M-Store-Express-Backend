...
describe('user controller test', () => {
  ...
  it('should update user', async () => {
    ...
    const updatedUser = (await usersService.updateUser(user._id, updateInfo)) as UserDocument;
  ...
  it('should create a new user if not found', async () => {
    ...
    const existedUser = (await usersService.findOrCreateUser(user, '')) as UserDocument;
    expect(existedUser.email).toBe(user.email);

    // Check with newly created user
    const expectedNewUser = (await usersService.findOrCreateUser(adminUser, '')) as UserDocument;
  ...
  it('should delete a user by user id', async () => {
    const user: UserDocument = await createUserInService(UserRole.Customer);
    const deletedUser = (await usersService.deleteUser(user._id)) as UserDocument;
  ...
  it('should update user password', async () => {
    ...
    const updatedUser = (await usersService.updateUser(user._id, user)) as UserDocument;
