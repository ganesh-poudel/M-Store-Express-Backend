...
describe('product controller test', () => {
  ...
  it('should create a product if user is admin', async () => {
    const productResponse = await createProductAndCategoryWithAuth(UserRole.Admin);
  ...
  it('should not create a product if user is customer', async () => {
    const productResponse = await createProductAndCategoryWithAuth(UserRole.Customer);
  ...
  it('should get a single product detail by id', async () => {
    ...
    const singleProductResponse = await request(app).get(`/api/v1/products/${productData._id}`);

    const singleProductData: ProductDocument = singleProductResponse.body;

    expect(singleProductResponse.status).toBe(200);
    expect(singleProductData).toHaveProperty('name', productData.name);
    expect(singleProductData).toHaveProperty('description', productData.description);
  });

  it('should update a product if user is admin', async () => {
    // create product with auth
    const productOriginalResponse = await createProductAndCategoryWithAuth(UserRole.Admin);
  ...
  it('should delete a product if user is admin', async () => {
    ...
    const productResponse = await createProduct(accessToken, categoryData.body._id);
